...
def diagonal_difference(arr):
    n = len(arr)  # longitud de la matriz
    left_to_right = 0  # suma de los elementos de la diagonal de izquierda a derecha
    right_to_left = 0  # suma de los elementos de la diagonal de derecha a izquierda

    # recorremos la matriz con un for loop
    for i in range(n):
        left_to_right += arr[i][i]  # sumamos los elementos de la diagonal principal
        # Obs: fila x columna = [i][n - i - 1] = [i][n - (i + 1)]
        right_to_left += arr[i][n - (i + 1)]  # sumamos los elementos de la antidiagonal

    # para devolver un valor absoluto usamos la funcion abs()
